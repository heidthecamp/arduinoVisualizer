...
#define SCOPE_PERIOD    (10 * NUM_TLCS)
#define LED_PERIOD      SCOPE_PERIOD / (NUM_TLCS * 16)

// *MODIFY NUM_TLCS IN tlc_config.h

TLC_CHANNEL_TYPE highs;
TLC_CHANNEL_TYPE mids;
TLC_CHANNEL_TYPE lows;
...
void loop()
{
  ...
  //highs = (NUM_TLCS * 16) - ((NUM_TLCS * 16) / 3);
  tlc_addFade(highs,                      // led channel
              (analogRead(AUDIO_IN) * analogRead(AUDIO_IN) / (analogRead(ANALOG_PIN) + 100)) * INTENSITY,   // start fade value (0-4095)
  ...
  if (highs++ == NUM_TLCS * 16) {
    highs = (NUM_TLCS * 16) - ((NUM_TLCS * 16) / 3);
  }
  
  
  //mids = (NUM_TLCS * 16) / 3;
  tlc_addFade(mids,                      // led channel
              (analogRead(AUDIO_IN) * analogRead(AUDIO_IN) / (analogRead(ANALOG_PIN) + 100)) * INTENSITY,   // start fade value (0-4095)
              0,                            // end fade value (0-4095)
              lastMillis + 2,               // start millis
              lastMillis + (uint16_t)SCOPE_PERIOD / 4  // end millis
             );
  if (mids++ == (NUM_TLCS * 16) - ((NUM_TLCS * 16) / 3)) {
    mids = (NUM_TLCS * 16) / 3;
  }

  tlc_addFade(lows,                      // led channel
              (analogRead(AUDIO_IN) * analogRead(AUDIO_IN) / (analogRead(ANALOG_PIN) + 100)) * INTENSITY,   // start fade value (0-4095)
              0,                            // end fade value (0-4095)
              lastMillis + 2,               // start millis
              lastMillis + (uint16_t)SCOPE_PERIOD / 4  // end millis
             );
  if (lows++ == (NUM_TLCS * 16) / 3) {
    lows = 0;
  }
